---
title: "Lab3:Normal Distribution"
author: "Zhan_Zhao"
date: "2025-09-22"
...
```{r}
#Using R as a Normal Distribution Calculator:
#Several R commands ending in norm(), e.g.,  pnorm(), qnorm(), dnorm(), calculate quantities for normal distributions. 

#Calculating probabilities/area under the Normal density curve to the left of the z-score
#Syntax:pnorm(z-score)
#Example: 
pnorm(1.96)
#p for cumulative probability of normal distribution

#Finding the critical z-score for a given area under the curve to the left of the z-score 
#(i.e. reading Table A backwards):
#Syntax:qnorm(probability)
#(q for quantile of normal distribution )
#Example:
qnorm(0,975)

#Note:Commands like pnorm() and qnorm() as listed above provide quantities for the standard normal distribution(mean=0,standard deviation = 1).
#To provide the equivalent quantities for another normal distribution, you would specify,e.g.,

#Syntax:pnorm(z-score,mean,sd)

#Example
pnorm(78,50,25)
#the probability/area under the curve to the left of 78 for a normal distribution with mean = 50 and standard deviation = 25)
```
